...
| `--danger` | `#7A3328` | Error text and review/booking error states |
...
- `BookingManagement`: current-user booking list, role/status actions, review submission for completed student bookings.
...
- `/dashboard`: stat cards, recent booking list, status badges, and manage-bookings link.
- `/search`: filter panel and alumni card grid.
- `/profile`: current-user profile summary from local auth storage.
- `/profile/[id]`: authenticated alumni detail card with student booking CTA, average rating, and review list.
- `/bookings`: student booking request form plus current-user booking management; alumni see booking management only.
...
- Loading states exist on dashboard, search, booking alumni loading, booking management, review loading, and saved alumni.
...
- Add notification UI.
- Add availability management UI for alumni.
